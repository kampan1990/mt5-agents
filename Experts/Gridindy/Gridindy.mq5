...
input bool   InpShowDashboard      = true;   // แสดงแดชบอร์ดบนชาร์ต
input int    InpDashX              = 10;     // ตำแหน่ง X (pixels จากขวา)
input int    InpDashY              = 30;     // ตำแหน่ง Y (pixels จากบน)
...
void DrawDashboard()
{
   ...
   int ox = InpDashX;   // offset จากขวา
   int oy = InpDashY;   // offset จากบน
   int W  = 220;        // ความกว้างแผง
   int lh = 16;         // ความสูงต่อบรรทัด
   int pad= 6;          // padding ซ้าย (จากขวา panel)
   ...
   DashLabel("state_lbl", ox + W - 2, y+2, StateName(g_state), cBg, 8);
   ...
   DashLabel("gdir_v",  ox + W - 2,     y, gridCnt > 0 ? gridDir : "-", gridCnt > 0 ? gridDirClr : clrSilver, 8);
   y += lh;
   DashLabel("gcnt_l",  ox + pad,       y, "ไม้เปิด :", clrSilver, 8);
   DashLabel("gcnt_v",  ox + W - 2,     y, StringFormat("%d / %d", gridCnt, InpMaxGridOrders), clrWhite, 8);
   y += lh;
   DashLabel("gdst_l",  ox + pad,       y, "ระยะกริด:", clrSilver, 8);
   DashLabel("gdst_v",  ox + W - 2,     y, StringFormat("%.0f pts", gridDist), clrWhite, 8);
   y += lh;
   DashLabel("gpnl_l",  ox + pad,       y, "กำไร/ขาดทุน:", clrSilver, 8);
   DashLabel("gpnl_v",  ox + W - 2,     y, profitStr, profitClr, 8);
   y += lh;
   DashLabel("gtgt_l",  ox + pad,       y, "เป้ากำไร :", clrSilver, 8);
   DashLabel("gtgt_v",  ox + W - 2,     y, StringFormat("%.2f USD", InpProfitTarget), clrSilver, 8);
   y += lh + 3;
   ...
   DashLabel("rst_v",  ox + W-2,   y, recActive ? "ทำงาน" : "ปิด", recActive ? clrOrange : clrSilver, 8);
   y += lh;
   DashLabel("rcnt_l", ox + pad,   y, "ชั้น     :", clrSilver, 8);
   DashLabel("rcnt_v", ox + W-2,   y, StringFormat("%d / %d", recCnt, InpMaxRecoveryOrders), clrWhite, 8);
   y += lh;
   DashLabel("rdst_l", ox + pad,   y, "ระยะ Rec :", clrSilver, 8);
   DashLabel("rdst_v", ox + W-2,   y, StringFormat("%.0f pts  x%.1f", recDist, InpRecoveryMultiplier), clrSilver, 8);
   y += lh + 3;
   ...
   DashLabel("tst_v", ox + W-2,  y, trailOn ? "ใช้งาน" : "รอกำไร", trailOn ? clrLimeGreen : clrSilver, 8);
   y += lh;
   DashLabel("trd_l", ox + pad,  y, "ระยะ Trail:", clrSilver, 8);
   DashLabel("trd_v", ox + W-2,  y, StringFormat("%.0f pts / step %.0f", InpTrailDistance, InpTrailStep), clrSilver, 8);
   y += lh + 3;
   ...
   DashLabel("adx_v",  ox + W-2,  y, StringFormat("%.1f  (>%.0f)", adx, InpAdxThreshold), adxClr, 8);
   y += lh;
   DashLabel("ema_l",  ox + pad,  y, "EMA"+IntegerToString(InpEmaPeriod)+"  :", clrSilver, 8);
   DashLabel("ema_v",  ox + W-2,  y, StringFormat("%.2f", ema), clrSilver, 8);
   y += lh;
   color rsiClr = (rsi > 55) ? clrLimeGreen : (rsi < 45 ? clrTomato : clrSilver);
   DashLabel("rsi_l",  ox + pad,  y, "RSI      :", clrSilver, 8);
   DashLabel("rsi_v",  ox + W-2,  y, StringFormat("%.1f", rsi), rsiClr, 8);
   y += lh;
   DashLabel("atr_l",  ox + pad,  y, "ATR      :", clrSilver, 8);
   DashLabel("atr_v",  ox + W-2,  y, StringFormat("%.0f pts", atr_pts), clrSilver, 8);
   y += lh;
   DashLabel("trnd_l", ox + pad,  y, "เทรน     :", clrSilver, 8);
   DashLabel("trnd_v", ox + W-2,  y, trendStr, trendClr, 8);
   y += lh + 3;
   ...
   DashLabel("eq_v",  ox + W-2,  y, StringFormat("%.2f", equity), clrWhite, 8);
   y += lh;
   DashLabel("bal_l", ox + pad,  y, "Balance  :", clrSilver, 8);
   DashLabel("bal_v", ox + W-2,  y, StringFormat("%.2f", balance), clrSilver, 8);
   y += lh;
   DashLabel("dd_l",  ox + pad,  y, "Drawdown :", clrSilver, 8);
   DashLabel("dd_v",  ox + W-2,  y, StringFormat("%.1f%% (max %.0f%%)", dd, InpMaxDrawdownPct), ddClr, 8);
   y += lh;
   color spClr = (spread <= InpMaxSpreadPoints) ? clrLimeGreen : clrRed;
   DashLabel("sp_l",  ox + pad,  y, "Spread   :", clrSilver, 8);
   DashLabel("sp_v",  ox + W-2,  y, StringFormat("%d pts (max %d)", (int)spread, InpMaxSpreadPoints), spClr, 8);
