...
// Runner pool รวม M1+M2+M3 ไม่แยก magic
// วาง BE-SL เฉพาะเมื่อ pool ครบ RunKeepN ไม้เท่านั้น
// Runner ชน BE → ออกเป็น grid ปกติ → หา runner ใหม่แทนจนครบ N
input int    RunKeepN     = 5;      // จำนวน runner ที่ต้องการ (รวมทุก magic)
input int    RunBEPts     = 10;     // offset BE-SL เหนือ entry (points)
input double TPRun        = 50.0;   // TP runner รวม (USD) — ปิดทั้ง pool
...
// Runner pool รวมทุก magic — array เดียว
ulong  gRunTk[];          // tickets ของ runner ทุกตัว
bool   gRunPoolFull = false; // true เมื่อ pool ครบ RunKeepN ครั้งแรก
...
bool IsRunner(ulong tk) {
   for(int i = 0; i < ArraySize(gRunTk); i++) if(gRunTk[i] == tk) return true;
   return false;
}
...
double GetWinnerSum(int m, ulong bestTk) {
   ...
   int n = GetBestProfit(m, 999, pTk, pPf, true);
   double s = 0;
   for(int i = 0; i < n; i++) {
